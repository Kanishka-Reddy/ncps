...
class AdaptiveLTCCell(LTCCell):
    def __init__(self, *args, time_penalty: float = 0.01, ponder_epsilon: float = 0.01, time_limit: int = 100, **kwargs):
        ...
        self.dynamic_tau_net = nn.Sequential(
            nn.Linear(self.state_size + self.sensory_size + 1, 128),  # Combining state, input, and step count
            nn.ReLU(),
            nn.Linear(128, self.state_size),  # Output size is the same as state size for individual tau per neuron
            nn.Softplus()  # Ensuring that tau is positive
        )

    def _calculate_dynamic_tau(self, state, inputs, step_count):
        # Concatenate state, inputs, and step count into a single tensor
        step_count_tensor = torch.full((state.size(0), 1), step_count, device=state.device)
        combined_input = torch.cat([state, inputs, step_count_tensor], dim=1)

        # Pass the combined input through the neural network to get dynamic tau
        dynamic_tau = self.dynamic_tau_net(combined_input)
    ...
    def _ode_solver(self, inputs, state, elapsed_time):
        ...
        # Pre-compute sensory neuron effects
        sensory_w_activation = self.make_positive_fn(
            self._params["sensory_w"]
        ) * self._sigmoid(
            inputs, self._params["sensory_mu"], self._params["sensory_sigma"]
        ...
        for t in range(self._ode_unfolds):
            if step_count >= self.time_limit or remaining_budget <= 0:
                break

            # Calculate dynamic time constants for this step
            dynamic_tau = self._calculate_dynamic_tau(state, inputs, step_count)

            # ODE dynamics with dynamic tau
            w_activation = w_param * self._sigmoid(
                state, self._params["mu"], self._params["sigma"]
            )
            ...
            gleak = self.make_positive_fn(self._params["gleak"])
            numerator = cm_t / dynamic_tau * state + gleak * self._params["vleak"] + w_numerator
            denominator = cm_t / dynamic_tau + gleak + w_denominator

            # Update state
            state = numerator / (denominator + self._epsilon)

            # Check halting condition
            p_continue = self.halting_unit(state).squeeze()
            step_halt = (remaining_budget - p_continue) > 0
            remaining_budget -= p_continue.item()

            # Update last_state and step_count
            last_state = state.clone() if step_halt else last_state
            step_count += 1
        return last_state
...
class AdaptiveLTC(nn.Module):
    ...
    def forward(self, input, hx=None):
        ...
        for t in range(seq_len):
            inputs = input[:, t] if self.batch_first else input[t]

            h_out, hx, ponder_cost, step_count = self.rnn_cell(inputs, hx)

            if self.return_sequences:
                output_sequence.append(h_out.unsqueeze(seq_dim))
                ponder_cost_sequence.append(ponder_cost.unsqueeze(0).unsqueeze(0))
                step_count_sequence.append(torch.tensor([step_count], device=device).unsqueeze(0))

        if self.return_sequences:
            outputs = torch.cat(output_sequence, dim=seq_dim)
            ponder_costs = torch.cat(ponder_cost_sequence, dim=seq_dim)
            step_counts = torch.cat(step_count_sequence, dim=seq_dim)
        else:
            outputs = h_out
            ponder_costs = ponder_cost_sequence[-1]
            step_counts = step_count_sequence[-1]
