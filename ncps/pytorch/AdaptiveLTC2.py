...
class AdaptiveLTCCell(AdaptiveCellWrapper):
    ...
    def __init__(
            self,
            time_penalty: float,
            wiring,                     #(wiring should include input size and hidden size(number of units))
            in_features=None,
            input_mapping="affine",
            output_mapping="affine",
            ode_unfolds=6,
        ...
        super().__init__(
            rnn_cell, time_penalty, initial_halting_bias, ponder_epsilon, time_limit
        )
    ...
    def forward(self, inputs, hidden):
        # Add the first-step flag to the input tensor
        batch_size = inputs.size(0)
        first_step_flag = torch.ones(batch_size, 1, device=inputs.device)
        inputs_with_flag = torch.cat([inputs, first_step_flag], dim=-1)

        # Call the forward method of AdaptiveCellWrapper
        return super().forward(inputs_with_flag, hidden)
