...
class ACTLTCCell(LTCCell):
    def __init__(self, *args, max_computation=10, epsilon=0.01, **kwargs):
        super(ACTLTCCell, self).__init__(*args, **kwargs)
        self.max_computation = max_computation
        self.epsilon = epsilon
        self.ponder_weight = nn.Parameter(torch.rand(1))  # Initialize ponder weight
    ...
    def forward(self, inputs, states, elapsed_time=1.0):
        ...
        remainders = []
        outputs = []
        for _ in range(self.max_computation):
            # Run the ODE solver (like the original LTCCell)
            next_state = self._ode_solver(inputs, states, elapsed_time)
            outputs.append(next_state)

            # Ponder gate to decide whether to continue computing
            p = torch.sigmoid(self.ponder_weight * next_state)
            update_gate = torch.le(p, self.epsilon).float()
            remainder = 1.0 - p
            ponder_costs.append(update_gate)
            remainders.append(remainder)

            if torch.all(update_gate).item():
                break  # Stop if all elements in update_gate are True

        # Calculate ponder cost
        ponder_cost = torch.stack(ponder_costs).sum() + torch.stack(remainders).sum()

        # Use the last state for the final output
        final_output = self._map_outputs(outputs[-1])

        return final_output, outputs[-1], ponder_cost
